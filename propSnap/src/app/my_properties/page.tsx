...
async function handleDelete(id:string,listedById:string) {
    try {
        await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/property/delete_Property`,{
            headers:{
  ...
  const fetchProperties = async () => {
    try {
      ...
      const response = await axios.get<ApiResponse>(
        `${process.env.NEXT_PUBLIC_API_URL}/property/my_properties`,
        {
          headers: {
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="space-y-6">
          {properties.map((property) => (
            <PropertyCard
            onDelete={()=>{handleDelete(property.id,property.listedById)}}
            showActions={true}
            type="my_property"
            id={property.id}
              key={property.id}
